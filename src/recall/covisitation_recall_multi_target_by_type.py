...
DEFAULT_COVIS_FILE = "multi_target_covis_topk_by_type.pkl"
DEFAULT_OUTPUT_FILE = "multi_target_covisitation_by_type_predictions.csv"
DEFAULT_K = 20


def parse_args(argv=None):
    parser = argparse.ArgumentParser(description="Generate type-specific multi-target co-visitation recall.")
    parser.add_argument(
        "--train-file",
    ...
    parser.add_argument(
        ...
        help=f"Type-specific co-visitation pickle file under outputs/. Default: {DEFAULT_COVIS_FILE}",
    )
    parser.add_argument(
        "--output-file",
...
def load_inputs(output_dir, train_file, labels_file, covis_file):
    ...
    with open(covis_path, "rb") as f:
        covis_topk_by_type = pickle.load(f)

    return train_events, valid_labels, covis_topk_by_type


def build_session_recommendations(valid_sessions, session_items, covis_topk_by_type, k):
    recs_by_session_type = {}

    for event_type, covis_topk in covis_topk_by_type.items():
        recs_by_session = {}
        for session in tqdm(valid_sessions, desc=f"Generating {event_type} covis recall"):
            recs = recommend(session_items.get(session, []), covis_topk, k)
            recs_by_session[session] = " ".join(map(str, recs))
        recs_by_session_type[event_type] = recs_by_session

    return recs_by_session_type


def build_predictions(valid_labels, recs_by_session_type):
    rows = []
    ...
    for session, event_type in zip(valid_labels["session"], valid_labels["type"]):
        rows.append({
            "session": session,
            "type": event_type,
            "predictions": recs_by_session_type.get(event_type, {}).get(session, ""),
        })
...
def main(argv=None):
    ...
    train_events, valid_labels, covis_topk_by_type = load_inputs(
        output_dir,
        args.train_file,
        args.labels_file,
        args.covis_file,
    ...
    recs_by_session_type = build_session_recommendations(
        valid_sessions,
        session_items,
        covis_topk_by_type,
        args.k,
    )
    predictions = build_predictions(valid_labels, recs_by_session_type)
    predictions.to_csv(output_dir / args.output_file, index=False)

    empty_rows = int((predictions["predictions"] == "").sum())
    print(f"Type-specific multi-target covisitation predictions saved to {args.output_file}")
